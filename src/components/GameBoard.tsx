...
const GameBoard = () => {
  const { state, selectPiece, deselectPiece, movePiece, getValidMovesForPiece } = useGame();
  const { board, pieces, selectedPiece, currentPlayer, lastMovedPieceColor } = state;
  ...
  return (
    <div className="w-full max-w-xl mx-auto">
      <div 
        className={cn(
          "grid grid-cols-8 gap-0.5 border-2 rounded-lg overflow-hidden",
      ...
      >
        {board.map((row, rowIndex) =>
          row.map((cellColor, colIndex) => {
            ...
            const isValidMoveCell = selectedPiece && getValidMovesForPiece(selectedPiece).some(
            ...
            return (
              <BoardCell
                ...
                isSelected={selectedPiece?.position.row === rowIndex && selectedPiece?.position.col === colIndex}
              >
                {piece && (
                  <GamePiece
                    piece={piece}
